...
class Restaurants {
  constructor() {
    this.restaurants = JSON.parse(
      fs.readFileSync("./app/data/restaurants.json", "utf-8")
    );
  }
  ...
  getAllRestaurantsByCategory(category) {
    try {
      ...
      const selectedCategoryFooditems =
        fooditems.getFooditemsByCategoryId(categoryId);

      // Step 3 : Find ids of the food items matching the selected category
      const selectedCategoryFooditemsIds = selectedCategoryFooditems.map(
      ...
      const selectedCategoryMenuIds = selectedCategoryMenuitems.map(
  ...
  getAllRestaurantsByCuisine(cuisine) {
    try {
      ...
      const selectedCuisineFooditems =
        fooditems.getFooditemsByCuisineId(cuisineId);

      //Step 3 : Find ids of the fooditems matching the selected cuisine
      const selectedCuisineFooditemsIds = selectedCuisineFooditems.map(
        (fooditem) => fooditem.id
      );
      ...
      const selectedCuisineMenuIds = selectedCuisineMenuitems.map(
        (menuitem) => menuitem.menuId
      );
    ...
    return `No Restaurants serving ${cuisine} cuisines`;
  }
}
