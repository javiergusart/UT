//1. Create a variable `isHappy` and assign a boolean value based on the value of the given String variable `action`. Check whether action variable value is `Smile` or not.

var action = "Smile";
var isHappy = action === "Smile";

console.log(isHappy);

// 2. Create a variable `allSubjects` and assign it a string representing concatenation of other string variables indicating various subjects.

var subject1 = "history";
var subject2 = "math";
var subject3 = "biology";
var allSubjects = subject1 + " " + subject2 + " " + subject3;

console.log(allSubjects);

// 3. Write a program to compare two numbers, `num1` and `num2`, and check if `num1` is greater than or equal to `num2`.

var num1 = 10;
var num2 = 5;

var comparison1 = num1 == num2;
var comparison2 = num1 >= num2;

console.log(comparison1);
console.log(comparison2);

// 4. Write a program to calculate the square of a given number, `num`.

var num = 4;
var square = num * num;

console.log(square);

// 5. Write a program to check if a given number, `num`, is of type number. If true check for even or odd. If false print "Invalid input".

var num = 7;

if (typeof num === "number") {
  if (num % 2 === 0) {
    console.log("Even");
  } else {
    console.log("Odd");
  }
} else {
  console.log("Invalid input");
}

// 6. Write a program to check if a given year, `year`, is a leap year.
//  Logic: number must be divisible by 400 or divisible by 4 but not divisible by 100.

var year = 2024;
var days = 0;

if ((year % 4 === 0 && year % 100 !== 0) || year % 400 === 0) {
  days = 366;
} else {
  days = 365;
}

console.log("Number of days in the year " + year + ": " + days);

// 7. Write a program that checks if a given character, `char`, is a vowel or a consonant.

var char = "a";

if (
  char === "a" ||
  char === "e" ||
  char === "i" ||
  char === "o" ||
  char === "u"
) {
  console.log(char + " is a vowel");
} else {
  console.log(char + " is a consonant");
}

// 8. Write a program that determines the largest among three numbers, `num1`, `num2`, and `num3`.

var num1 = 10;
var num2 = 5;
var num3 = 8;
var largest;

if (num1 >= num2 && num1 >= num3) {
  largest = num1;
} else if (num2 >= num1 && num2 >= num3) {
  largest = num2;
} else {
  largest = num3;
}

console.log("the largest is:", largest);

// 9. Write a program that determines the sign of a given number, `num` (positive, negative, or zero), using the ternary operator.

var num = -5;

var sign = num > 0 ? "positive" : num < 0 ? "negative" : "zero";

console.log("The number is:", sign);

// 10. Write a program that determines the grade based on a given percentage, `percentage`. Use the following grading scale: A (90-100), B (80-89), C (70-79), D (60-69), F (0-59).

var percentage = 85;
var grade;

if (percentage >= 90 && percentage <= 100) {
  grade = "A";
} else if (percentage >= 80 && percentage <= 89) {
  grade = "B";
} else if (percentage >= 70 && percentage <= 79) {
  grade = "C";
} else if (percentage >= 60 && percentage <= 69) {
  grade = "D";
} else if (percentage >= 0 && percentage < 60) {
  grade = "F";
} else {
  grade = "Invalid percentage";
}

console.log("Grade:", grade);
